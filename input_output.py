...
class FileStruct:
    def __init__(self, audio_file=None, midi_file=None):
        if audio_file is not None:
            self.audio_file = Path(audio_file)
            self.track_name = self.audio_file.stem
            self.ds_path = self.audio_file.parents[1]
            self.midi_file = None
        elif midi_file is not None:
            self.midi_file = Path(midi_file)
            self.track_name = self.midi_file.stem
            self.ds_path = self.midi_file.parents[1]
            self.audio_file = self.ds_path.joinpath(
                'audio', self.track_name + '.wav')
        else:
            raise ValueError("No valid audio or midi file.")
        ...
        # Bar Index, Repition, Tempo, Time Signature
        self.info_file = self.ds_path.joinpath('info', self.track_name
                                               + '.json')
    ...
    #     return "FileStruct(\n\tds_path=%s,\n\taudio_file=%s,\n\test_file=%s," \
    #         "\n\json_file=%s,\n\tref_file=%s\n)" % (
    #             self.ds_path, self.audio_file, self.test_file,
    ...
    def __repr__(self):
        ...
        return "FileStruct(\n\tds_path=%s,\n\taudio_file=%s" \
            "\n\json_file=%s,\n\tref_file=%s\n)" % (
                self.ds_path, self.audio_file, self.json_file, self.ref_file)
    ...
    if not hier:
        ...
        for lower in lowers:
            if lower['annotation_metadata']['annotator'] == annotator:
                ...
                list_ref_times.append(
                    utils.intervals_to_times(ref_inters_lower))
...
def read_references_2annot(audio_path, index):
    ...
    #
    ref_inters_upper, ref_labels_upper = utils.remove_empty_segments(
    ...
    ref_inters_lower, ref_labels_lower = utils.remove_empty_segments(
...
def read_references_jsd(audio_path, level):
    ...
    (ref_int, labels) = mir_eval.util.adjust_intervals(
        ref_int, labels, t_min=0, t_max=duration)
...
def write_beats(beat_times, file_struct, feat_config):
    ...
    if json_file.exists() and os.path.isfile(json_file):
        ...
        out_json = utils.create_json_metadata(
            file_struct.audio_file, 0, feat_config)
