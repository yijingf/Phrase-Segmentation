...
class Triplet_features_dset(torch.utils.data.Dataset):
    ...
    def __init__(self, experiment_config, W=16, ALPHA=60, BETA=.85, alpha_neg=5, LAMBDA=.5, tracklist=None):
        ...
        self.LAMBDA = LAMBDA
        self.feature_types = experiment_config.feature_types
        self.tracklist = clean_tracklist(
            experiment_config, annotations=False, tracklist=tracklist)  # beat & feat files required
    ...
    def buildTriplets_(self):
        ...
        for track in tqdm(tracks):
            ...
            mfcc = get_features(
                track, self.feature_types[0], self.experiment_config, self.experiment_config.feat_type)
            chromas = get_features(
                track, self.feature_types[1], self.experiment_config, self.experiment_config.feat_type)
            mfcc_synced = librosa.util.sync(
                mfcc, beat_frames, aggregate=np.mean)[1:]
            mfcc_synced = ndimage.median_filter(mfcc_synced, size=(1, 8))
            chromas_synced = librosa.util.sync(
                chromas, beat_frames, aggregate=np.mean)
            chromas_synced = ndimage.median_filter(chromas_synced, size=(1, 8))
            SSM, SSM_neg = self.compute_ssms(mfcc_synced, chromas_synced)
            triplets_track = self.get_triplets(
                track, SSM, SSM_neg, n_per_songs, beat_frames)
    ...
    def buildTriplets(self):
        ...
        print('Sampling triplets...')
        triplets = []
        jobs = [joblib.delayed(self.sample_triplets_track)(
            track=track, n_per_songs=n_per_song) for track in tracks]
        out = joblib.Parallel(n_jobs=16, verbose=1)(jobs)
    ...
    def get_triplets(self, track, SSM, SSM_neg, n_per_songs, beat_frames):
        ...
        try:
            L = len(SSM)
            try:
                anchors_total = np.random.choice(
                    np.arange(len(SSM)), size=n_per_songs, replace=False)
            except:
                anchors_total = np.random.choice(
                    np.arange(len(SSM)), size=n_per_songs, replace=True)
            for anchor in anchors_total:

                positive = np.random.choice(
                    np.arange(len(SSM)), size=1, p=SSM[anchor] / np.sum(SSM[anchor]))[0]

                negative = np.random.choice(
                    np.arange(len(SSM)), size=1, p=SSM_neg[anchor] / np.sum(SSM_neg[anchor]))[0]

                triplets_track.append(
                    [track, (beat_frames[anchor], beat_frames[positive], beat_frames[negative])])
    ...
    def compute_ssm(self, X, delay):
        if delay != 0:
            ...
            R_pos = librosa.segment.recurrence_matrix(
                X_stack, mode='affinity', width=2, k=X.shape[1]).astype(float)
            for i in range(len(R_pos)):
                R_pos[i] = (R_pos[i] - np.min(R_pos[i])) / \
                    (max(np.max(R_pos[i]) - np.min(R_pos[i]), 1e-6))
                R_pos[i, i] = 0
            return R_pos
        else:
            R_pos = librosa.segment.recurrence_matrix(
                X, mode='affinity', width=2, k=X.shape[1]).astype(float)
            for i in range(len(R_pos)):
                R_pos[i] = (R_pos[i] - np.min(R_pos[i])) / \
                    (max(np.max(R_pos[i]) - np.min(R_pos[i]), 1e-6))
                R_pos[i, i] = 0
    ...
    def compute_ssms(self, mfcc, chroma):
        ...
        for i in range(len(SSM_chromas)):
            SSM_chromas[i] = self.sigmoid(
                SSM_chromas[i], self.ALPHA, self.BETA)
        ...
        for i in range(len(SSM)):
            SSM[i] = (SSM[i] - np.min(SSM[i])) / \
                (max(np.max(SSM[i]) - np.min(SSM[i]), 1e-6))
        ...
        for i in range(len(SSM_gaussian)):
            SSM_gaussian[i] = (SSM_gaussian[i] - np.min(SSM_gaussian[i])) / \
                (max(np.max(SSM_gaussian[i]) - np.min(SSM_gaussian[i]), 1e-6))

        # exponential decay
        for i in range(len(negatives_gaussian)):
            for j in range(len(negatives_gaussian)):
                num = max(np.abs(i - j) / len(negatives_gaussian),
                          SSM_gaussian[i, j])
    ...
    def __getitem__(self, index):
        track, (anchor_index, positive_index,
                negative_index) = self.triplets[index]
        features = get_features(track, self.experiment_config.feat_id,
                                self.experiment_config, self.experiment_config.feat_type)
        features = (features - np.min(features)) / \
            (max(np.max(features) - np.min(features), 1e-6))
        features_padded = np.pad(features,
                                 ...
                                 mode='edge')
        anchor_patch = torch.as_tensor(
            features_padded[:, anchor_index:anchor_index + self.experiment_config.embedding.n_embedding][None, :, :])
        positive_patch = torch.as_tensor(
            features_padded[:, positive_index:positive_index + self.experiment_config.embedding.n_embedding][None, :, :])
        negative_patch = torch.as_tensor(
            features_padded[:, negative_index:negative_index + self.experiment_config.embedding.n_embedding][None, :, :])
