...
def train_epoch(model, loss_func, device, train_loader, optimizer, disable=False):
    ...
    for batch_idx, triplets in enumerate(tqdm.tqdm(train_loader, disable=disable)):
        ...
        loss_triplet = loss_func(
            embeddings_anchor_norm, embeddings_positive_norm, embeddings_negative_norm)
        loss = loss_triplet
        loss.backward()
        optimizer.step()
...
def train_model(exp_config):
    ...
    # Dataset
    train_data = Triplet_features_dset(
        exp_config, tracklist=exp_config.tracklist)
    ...
    train_loader = DataLoader(
        train_data, batch_size=exp_config.batch_size, shuffle=False, **dataloader_kwargs)
    ...
    optimizer = optim.SGD(
        parameters, lr=exp_config.learning_rate, momentum=0.9, weight_decay=1e-4)
