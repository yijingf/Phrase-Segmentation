...
def compute_features(y, feat_id, feat_config):
    ...
    if feat_id == 'mfcc':
        ...
        features = librosa.feature.chroma_cqt(
            y=y, sr=feat_config.sample_rate, fmin=27.5, n_octaves=8, hop_length=feat_config.hop_length)
...
def score_beats(info_file, duration, sr=22050):
    # Read beat times from score
    with open(info_file) as f:
        info = json.load(f)

    t_interval = [i['t'] for i in info['onset']]
    t_interval += [duration]

    beat_times = []
    for i, (t_st, t_ed) in enumerate(zip(*(t_interval[:-1], t_interval[1:]))):
        tempo = info['onset'][i]['tempo']
        beat_times += np.arange(t_st, t_ed, 60 / tempo).tolist()
        tempo = info['onset'][0]['tempo']

    if beat_times[-1] < duration:
        beat_times = np.append(beat_times, duration)
    beat_frames = librosa.time_to_frames(beat_times, sr=sr, hop_length=256)
    return beat_frames
...
def get_features(audio_file, feat_id, feat_config, feat_type, y=None,
    ...
    if valid_feat_files(file_struct, feat_id, feat_config):
        features = np.load(
            file_struct.get_feat_filename(feat_id), mmap_mode='r')
    else:
        if y is None:
        ...
        if save:
            ...
            write_features(features, file_struct, feat_id,
                           feat_config, beat_frames, duration)
    return features
...
def compute_beats(audio_file, y, feat_config):
    ...
    # Beat estimation
    if os.path.exists(file_struct.info_file):
        # Actual beat time
        beat_frames = score_beats(file_struct.info_file, duration)
    else:
...
def process_track(audio_file, feat_ids, feat_config):
    ...
    if not validate_all_files(file_struct, feat_ids, feat_config):
        ...
        y, _ = librosa.load(audio_file, sr=feat_config.sample_rate)
        for feat_id in feat_ids:
            get_features(file_struct.audio_file, feat_id,
                         feat_config, 'framesync', y=y, save=True)
    else:
